...
import { findProjectConfig } from '../command/cli/cli-port.js';
...
/**
 * `configWithInstallSource` lives in `@reticlehq/init` — `init` is the only thing that writes
 * `.reticle.json`, and the scaffolder may not import the daemon. Re-exported here so this module
 * stays the one place to read about install attribution, and so its tests are unchanged. Which
 * channel declared itself is still decided above; the plan receives the answer as
 * `PlanInput.installSource`.
 */
export { configWithInstallSource } from '@reticlehq/init';
