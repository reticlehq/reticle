 ...
 *
 * Scoped to `adapters/realm/browser/src` originally because that is where the incident happened. The
 * identical failure then arrived in `server/src`: a test timed out at vitest's 5s default
...
describe('heavy tests do not rely on the default timeout', () => {
  // Only the packages we publish. It used to read one directory, `packages/`, which stopped being
  // where they all are; reading the whole repository instead sweeps in scratch directories that
  // exist only to be probed by other checks, and it opened one that a check had already deleted.
  const sources = publishedPackageDirs()
  ...
  it('finds the IO-in-a-loop files, and only those — the rule is narrow on purpose', () => {
    ...
    expect(ioLoop.map(({ file }) => relative(REPO, file))).toHaveLength(EXPECTED_IO_LOOP_FILES);
  });
