...
export interface JournalReader {
  ...
  /**
   * How big the event ledger is right now, and the ceiling it is measured against.
   *
   * Synchronous and cached: the size is already tracked for the cap, so reporting it costs a field
   * read. Optional on the same reasoning as `readActions` -- a partial double declares no size,
...
/** The event ledger's size on disk against the ceiling that will refuse it. */
export interface JournalLedgerSize {
  /** Bytes of `events.jsonl` on disk. */
  bytes: number;
  /** The ceiling that refuses the next batch past it. */
  capBytes: number;
}
