...
describe('cloudFetch', () => {
  ...
  it('names the credential when it was pasted from a masked display, instead of leaking a ByteString error', async () => {
    // The real report: the API key was set to `rk_live_…` — the ellipsis-masked placeholder a
    ...
    // Names the CURRENT spelling, because that is what the reader should go and set.
    expect(message).toContain('RETICLE_API_KEY');
