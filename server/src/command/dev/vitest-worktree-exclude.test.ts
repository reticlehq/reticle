...
// These read the whole repository and start a second process, so they take about half a second on
// their own and considerably longer when every package's suite is running at once. The default
// five-second budget is a statement about how busy the machine is, not about this code, and a check
// that only fails on a loaded runner teaches people to press re-run instead of to look.
describe('root vitest excludes agent worktrees', { timeout: 60_000 }, () => {
  it(
