...
**Before coding on a GitHub issue: `pnpm check:open-prs`, and read the PR list for the SYMPTOM, not the issue number.** Contributors' titles rarely carry the number, so grepping the PR list for `#NNN` finds nothing — measured on 2026-09-08, when four people (two of them maintainers) duplicated each other in one day, and one maintainer's version was the _worse_ fix. The same command names open PRs whose work is already on `main`: ten were found that day, some open two weeks, one merged under a commit literally titled "Merge PR #645". It reports and never gates, because a title match is evidence, not proof — verify with `git merge-base --is-ancestor` before closing anybody's work.

**Do not push to `main` while a deep PR queue is open.** Every push sends every open PR `BEHIND`, and any that touched `CHANGELOG.md` goes conflict-dirty. With ten queued this turns one merge per CI cycle into a treadmill nobody can drain. Open a PR instead and let it queue on the same terms as everyone else's.
...
**A before/after measurement is invalid unless BOTH sides were built clean.** Measuring the browser bundle after lowering its `target` gave "48x smaller" — a half-finished build, not a result, and it was one sentence from being reported. `rm -rf dist` is not enough: `tsconfig.tsbuildinfo` survives it, so `tsc -b` decides it has nothing to do and emits nothing (including the `.d.ts` every dependent package then fails to find). Delete the buildinfo too, build both sides from clean, and if a number looks too good it is a broken build until proven otherwise. The honest figure was +2.6%.

**Never grep source for syntax — parse it.** A guard that matched `?.` / `??` / `??=` with regexes reported two false failures on output that was already correct: both hits were inside comments, one of them a comment quoting the very `form?.textContent` an earlier fix had removed. A regex cannot tell code from prose about code. Use the parser the consumer uses (acorn at the right `ecmaVersion`), and give it a negative control — a check nobody has watched reject something proves nothing when it passes.
