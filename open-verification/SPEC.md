...
A subject is what **persists** across the actions taken against it. Where a subject's unit of execution is shorter than the journey, a command that runs and exits, a job, a request handler, the execution is a WINDOW and MUST NOT be the `instance`: the subject is the durable thing those executions act upon, and `instance` MUST change when THAT is replaced. A command-line tool's subject is the installed tool together with the workspace it operates on. An implementation that names the execution as the instance makes every multi-step journey self-invalidating, and reports it as evidence superseded, which describes a fault in nothing. [OVP-SUBJ-3]
...
Independence is a property of a **channel**, not of an observation. An implementation whose vantage point sees facts of differing provenance MUST declare them as separate channels rather than as one channel at an averaged trustworthiness, and MUST NOT report an individual observation as more independent than the channel it arrived on. A process's chosen exit code and its termination by the operating system are the worked example: the first is the subject describing itself and the second is not. An implementation that merges them either lets a self-report buy a proof, or leaves a real crash unable to contradict one. [OVP-CHAN-5]
...
- **A consequence with no channel: an artifact the subject emitted.** A file a browser hands to the user, a file a command-line tool writes, a report a job renders. It is not `net` (nothing was transported), not `storage` (that is the subject's own durable state), and not `ui`. Implementations declare it as `x-artifact` today, and with different independence: a browser download is fully determined by the page that built it, while whether a write lands on a filesystem is decided by the filesystem. That is a per-realm declaration working as designed, and it is also why the name has not graduated: graduation needs two _independent_ implementations agreeing on its meaning, and the two that exist share an author.
