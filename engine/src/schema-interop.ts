...
/**
 * `@reticlehq/core` and `@reticlehq/engine` intentionally pin different `zod` versions: core is
 * capped below 3.23.0 so the browser SDK's transitive dependency tree stays parseable by webpack 4
 * (see `legacy-syntax-guard.test.ts`), while engine floats to satisfy its own newer dependents'
 * peer ranges. Each version is a SEPARATE installed copy of the `zod` package, so a schema core
 * built is an instance of core's `ZodObject` class, not engine's — two distinct classes with the
 * same name, one per module instance. `.parse`/`.safeParse`/`.extend` are plain method calls and
 * work identically either way, so most engine code never notices. `instanceof` does not: it
 * compares prototypes, and `coreSchema instanceof engineZod.ZodObject` is `false` even though the
 ...
 * check. Mirrors `server/src/surface/schema-interop.ts` — `server` cannot import from `engine`'s
 * bridge (nor the reverse) since they are separate packages with the identical cross-instance
 * problem against `core`.
 *
 * `asServerZodObject`/`asServerZodType` re-type a core-built schema as engine's own zod type at the
 * few points engine COMPOSES one (`.extend()`, `z.array()`, a generic `ZodTypeAny` parameter) —
 * TypeScript treats the two classes as structurally incompatible once a newer zod adds fields
 * (`~standard`/`~validate`) an older one lacks. These casts change only the type engine code sees;
 ...
 * `asServerZodType<ElementQuery>(ElementQuerySchema)` — instead of collapsing to `ZodTypeAny`'s
 ...
 * returns. Reading `.issues`/`.path`/`.code` works identically either way; only
 * `exactOptionalPropertyTypes` makes the two versions' `ZodError` nominally incompatible types.
 */
