# Competitive benchmark — Reticle vs the browser-agent field

> Measured, reproducible comparison of Reticle against the browser agents that claim **bug detection** or **token efficiency**. Every number here is produced by a script in `bench/harness/` (listed per section); nothing is hand-entered. Honest by design — the caveats and the cases where Reticle _loses_ are included, because a benchmark you can't trust is worthless.

## TL;DR

- **Detection:** Reticle is the only tool at **100%** on the injected-regression suite (10/10), 0 false positives. The cheapest tool (agent-browser) catches **72.7%** — cheap because it's blind.
- **Tokens:** in its recommended `hybrid` profile Reticle is **competitive** with the MCP substrates on a full flow (and cheaper than Playwright MCP), not the leanest — agent-browser is leaner _by being blind_. Reticle's real token win is the **regression loop: ~122 tokens/run** (deterministic replay) vs ~30k to re-drive with any LLM tool — **~248× per flow, ~4,172× at suite scale.**
- **Uniqueness:** no competitor combines **deterministic replay + app-signal assertions + a test harness** in one tool. The field is converging on record-once/replay, but all validate with shallow "non-empty/non-error" checks — **everyone can replay; only Reticle verifies.**
- **Honest limits:** Reticle is _blind to pure presentation/CSS bugs_ via signals (its opt-in visual-diff layer covers those); needs an embedded SDK (zero-config for DOM/network/console, ~20 one-line `reticle.signal()` calls for the signal oracle); and is a verifier for _your_ app, not a web navigator.
...
**Silent-regression sub-score** (the bugs that render fine — KPI card removed, layout shift, broken validation): **Reticle 3/3; Playwright MCP 2/3; DevTools 1/3; Playwright CLI 1/3; agent-browser 0/3.** Only Reticle catches all three.
...
| **Reticle (hybrid — recommended default)** | ~23,000                            |
| Chrome DevTools MCP                        | 24,622                             |
| Playwright MCP                             | 25,493                             |
| Reticle (full — current default)           | 45,088                             |

> **Actionable:** Reticle's _current_ default (`full`, 57 tools) carries a 16k-token schema tax — the highest of any tool — making it the most expensive on a full flow. The `hybrid` profile (14 tools, 5.7k schema, −64%) makes Reticle cheaper than Playwright MCP **with no detection loss** (verified: 10/10 in `hybrid`) and flow-replay reachable via the `reticle_run` meta-tool. See `schema-tax.mjs`.

| Profile        | Tools | Schema tokens | Detection oracles | Flow-replay       |
| -------------- | ----- | ------------- | ----------------- | ----------------- |
| dynamic        | 2     | 256           | on-demand         | on-demand         |
| **hybrid**     | 14    | **5,748**     | direct            | via `reticle_run` |
...
|                    | Reticle (deterministic replay) | Competitor (LLM re-drive) | Ratio   |
| ------------------ | ------------------------------ | ------------------------- | ------- |
| 1 flow / run       | **122 tok**                    | ~30,249 (Playwright MCP)  | ~248×   |
| 4-flow suite / run | **29 tok** (one verdict)       | ~120,996                  | ~4,172× |

`reticle_flow_verify` returns one verdict for K flows → read-cost ~constant in K; competitors re-drive each flow with the LLM every run. The ratio compounds with suite size.
...
| **Reticle app-signal**                 | **4 / 4** |

Visual diff sees only pixels; a network-mock replays yesterday's response so the live backend change never reaches it (Meticulous's own words: "only catches frontend regressions"). Reticle asserts on the live payload/signal.

## 6. Oracle-coverage — best-of-both (`oracle-coverage-bench.mjs`)

| Tool                                       | Backend-contract | Presentation / CSS | Classes |
| ------------------------------------------ | ---------------- | ------------------ | ------- |
| **Reticle** (signal + visual diff)         | COVERED          | COVERED            | **2/2** |
| every substrate / Meticulous / Antigravity | BLIND            | COVERED            | 1/2     |

Reticle carries both oracle families. _Honest:_ the signal oracle is blind to pure CSS bugs — Reticle needs its visual layer there (it has it). The claim is **best-of-both coverage**, not "signals beat pixels."

## 7. Capability matrix

| Capability | Reticle | PW MCP | DevTools MCP | agent-browser | PW CLI | Stagehand | Browser Use | Meticulous |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
