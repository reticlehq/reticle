...
if (matrix) {
  ...
  const R = buggy('reticle-script'),
    P = buggy('playwright-script');
  ...
  const rFP = rows.filter(
    (r) => r.harness === 'reticle-script' && r.variant === 'clean' && r.caught,
  ).length;
  const pFP = rows.filter(
    (r) => r.harness === 'playwright-script' && r.variant === 'clean' && r.caught,
  ).length;
  const total = R.length;
  const byCat = {};
  for (const r of rOnly) (byCat[r.category] ??= []).push(r.bug);
  const rMs = R.reduce((a, r) => a + r.ms, 0),
    pMs = P.reduce((a, r) => a + r.ms, 0);

  L.push('## #1 — "Playwright already catches everything for us"\n');
  L.push(
    `Of **${total}** bugs, **${rOnly.length}** are caught by Reticle and **missed by Playwright 100% of the time** — structurally, not occasionally: the truth lives in the app's store / commit-stream / network cardinality, which a DOM+pixel tool cannot read. That's **${n((rOnly.length / total) * 100)}%** of the catalog.\n`,
  );
  L.push(
  ...
  L.push('|---|--:|--:|');
  L.push(
    `| Bugs caught (of what it *can* catch) | ${R.filter((r) => r.caught).length}/${total} | ${P.filter((r) => r.caught).length}/${total} |`,
  );
...
if (multi) {
  const r = multi.reticle,
    p = multi.playwright;
  L.push(`Workload: ${multi.workload} bugs, concurrency ${multi.levels.join(' vs ')}.\n`);
  L.push('| Tool | C=1 | C=3 | speedup | caught |');
  L.push('|---|--:|--:|--:|--:|');
  L.push(
    `| Reticle | ${n(r['1'].ms / 1000, 1)}s | ${n(r['3'].ms / 1000, 1)}s | ${n(r['3'].speedup, 2)}× | ${r['3'].caught}/${multi.workload} |`,
  );
  L.push(
    `| Playwright | ${n(p['1'].ms / 1000, 1)}s | ${n(p['3'].ms / 1000, 1)}s | ${n(p['3'].speedup, 2)}× | ${p['3'].caught}/${multi.workload} |`,
  );
  L.push(
...
if (mcp) {
  ...
  L.push(
    `| Avg $ / bug | $${n(a.playwright_mcp.avgCostUsd, 4)} | **$${n(a.reticle.avgCostUsd, 4)}** |`,
  );
  L.push(
    `| clean-build false alarms | ${a.playwright_mcp.falsePositives} | ${a.reticle.falsePositives} |`,
  );
  L.push(
  ...
  L.push(
    `> Caveat: the gpt-4o+Reticle agent over-flagged clean builds (${a.reticle.falsePositives} false alarms) — an agent-behavior artifact (under-investigation + our force-a-verdict prompt), NOT Reticle's data: the deterministic Reticle-script had 0 false positives. A stronger model (Claude) and better prompting remove it. The robust, model-independent signals are tokens, turns, and $.\n`,
  );
...
if (author) {
  ...
  L.push(
    "> The trade: a Playwright script amortizes over repeated runs (deterministic replay), which is exactly Reticle's `reticle_flow_verify` too — recorded once, replayed free. Reticle skips the authoring step for both one-off checks and recorded regressions.\n",
  );
} else L.push('_not run_\n');
...
L.push(
  "`?opaque=2` strips `data-testid` + `role` + `aria` (leaving only text + Reticle's dev-only source stamps) — the div-soup / hashed-class / obfuscated-build reality where DOM-selector tools lose their grip.\n",
);
L.push(
  "**Measured, and the honest result:** under `?opaque=2`, `reticle_state` **still returned the live store** (`deployments.0.status`, `deployments.0.service` read back cleanly) — Reticle's state verification is **100% DOM-independent**, so a fully opaque shell does not touch it. All 14 state/business bugs stay Reticle-exclusive in *any* shell, because Playwright has no app-state access in the first place.\n",
);
L.push(
  'The scripted *trigger* (clicking an icon-button by visible text) was unreliable once `role` was stripped — but that limit hit **both** scripted tools equally (a harness-anchoring artifact, not a Reticle capability gap; an LLM agent or role/source anchoring drives the trigger fine, and the hard part — reading the store — already works opaque). Net: **an opaque shell can only *hurt* Playwright** (its entire toolkit is DOM selectors); Reticle keeps its state reads (anchor-free) and its component:file:line source stamps (stable when classes hash).\n',
);
if (opaque)
  L.push(
    '> Reticle read the store under a fully-stripped DOM: proof the state layer needs no selectors at all.\n',
  );

L.push('## #6-authentic — Claude Code as harness (measured, hardened bug `kpi-deploys-tamper`)\n');
L.push(
  'Claude subagents drove each MCP to verify the same never-rendered-field tamper (`deployments.0.costUsd`). **Both caught it** — the honest result:\n',
);
