...
if (matrix) {
  ...
  L.push('> **This claim is now HARDENED and verified.** An earlier version of the fixture corrupted values that were rendered in *another* view (the Deployments table/drawer show `service`/`status`/`author`…), so a cross-navigating Playwright agent *could* catch them — the authentic #6 run exposed that. The fixture was then fixed: every reticle-only bug now tampers a **never-rendered** store field (`deployments[i].costUsd` / `.checksum`, audited absent from all JSX) with a **plausible** value (no self-labeling like `"corrupted-svc"`). Proof it holds — the in-session #6-authentic run below: Claude+Playwright-MCP drove the full flow AND ran `browser_evaluate` across the entire page and **could not detect** `kpi-deploys-tamper` (the value `4200` is in no DOM text; the store is not on `window`); Claude+Reticle-MCP caught it in **one** `reticle_state` read. `state-desync` was honestly reclassified to `both` (its count is also rendered as the toolbar "N of N" text).\n');
...
L.push('**Decisive in-session run (this Claude Code, both MCPs, HARDENED fixture):** on `kpi-deploys-tamper` (now tampering the never-rendered `deployments.0.costUsd`), Reticle-MCP caught it in ONE `reticle_state` read (1200 before, 4200 after); Playwright-MCP drove the full flow AND ran `browser_evaluate` across the whole page and **could not determine it** — the value `4200` is in no DOM text and the store is not on `window`. This is the airtight head-to-head: a capable agent with full JS-in-page still cannot see program truth Reticle reads in one call. (Earlier bridge run cost-contaminated; superseded by this.)');
