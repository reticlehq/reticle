...
if (matrix) {
  ...
  L.push('> **CORRECTED — the honest claim is EFFICIENCY, not exclusivity.** Two rounds of authentic Claude-agent testing disproved "Playwright is blind": (1) a cross-navigating agent reads corrupted values rendered in *other* views; (2) even for values rendered in NO view (`deployments[i].costUsd`/`.checksum`), a Claude+Playwright-MCP agent caught the tamper by using `browser_evaluate` to **walk the React fiber** and read component state directly. `browser_evaluate` runs arbitrary page JS, so almost nothing is truly unreachable. **What survives is cost + robustness** (measured, same bug `kpi-deploys-tamper`, Claude driving each MCP): Reticle-MCP **4 tool calls / 35.6k tokens / 45s** via one typed `reticle_state` read; Playwright-MCP **45 tool calls / 86.7k tokens / 523s** reverse-engineering the fiber — and fragile (breaks on minified prod builds, non-React stores, fiber-shape changes). So the `${rOnly.length}` "reticle-only" is a *deterministic-harness* number; the real story is **Reticle reads program truth in one stable call; a DOM tool must spelunk internals at ~11× the calls/time.** `state-desync` was honestly reclassified to `both`.\n');
