  ...
  for (const bug of bugs) {
    for (const variant of ['clean', 'buggy']) {
      ...
      try {
        ...
        const seen = await page.evaluate(() => [...document.querySelectorAll('[data-testid]')].map((e) => e.getAttribute('data-testid')).slice(0, 12)).catch(() => []);
        const err = await page.locator('[data-testid="login-error"]').innerText().catch(() => '');
        ...
        if (c.kind === 'usable') {
          ...
          const info = ok ? await loc.evaluate((el) => {
            ...
            return { opacity: s.opacity, cursor: s.cursor, occluded: top !== el && !el.contains(top) };
          }).catch(() => null) : null;
          bytes += JSON.stringify({ box, info }).length;
          caught = !ok ? false : !box || box.width === 0 || box.height === 0 || (info && (info.opacity === '0' || info.occluded));
          note = ok ? `box=${box?.width}x${box?.height} opacity=${info?.opacity} occluded=${info?.occluded}` : 'element not found';
        } else if (c.kind === 'paint') {
          await sleep(400);
          const shot = await page.screenshot({ fullPage: false });
          bytes += shot.length;
          if (variant === 'clean') { baseline = shot; caught = false; note = `baseline captured (${shot.length}B)`; }
          else {
          ...
          const txt = await page.locator(`${sel(c.testid)} .nav-badge`).innerText().catch(() => '');
          bytes += txt.length;
      ...
      results.push({ harness: 'playwright-script', bug: bug.id, category: bug.category, variant, caught, expect: bug.expect, bytes, ms: Date.now() - t0, note });
    }
  }
