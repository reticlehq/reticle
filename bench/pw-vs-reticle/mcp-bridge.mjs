...
const tools = (await client.listTools()).map((t) => ({ name: t.name, desc: (t.description ?? '').slice(0, 180) }));

http
  .createServer((req, res) => {
    if (req.url === '/tools') { res.writeHead(200, { 'content-type': 'application/json' }); res.end(JSON.stringify(tools)); return; }
    if (req.url === '/call' && req.method === 'POST') {
      let b = ''; req.on('data', (c) => (b += c));
      req.on('end', async () => {
        try { const { tool: tn, args } = JSON.parse(b || '{}'); const out = await client.callTool(tn, args ?? {}, 60000); res.writeHead(200); res.end((out.text ?? '').slice(0, 6000)); }
        catch (e) { res.writeHead(200); res.end('ERR ' + String(e).slice(0, 200)); }
      });
      return;
    }
    res.writeHead(200); res.end('bridge:' + tool);
  })
