// Source localization (Iris-only on the component hierarchy): "which component renders this element,
// and where is it defined?" — the answer an agent needs to FIX a bug, not just locate a pixel.
//
// Iris walks the React fiber from a DOM node up to the root and returns the COMPONENT STACK
...
// a CSS selector / role+name. (Honest caveat: the babel plugin also stamps a `data-iris-source`
...
try {
  ...
  for (const testid of TARGETS) {
    ...
    const ins = parse((await a.c.callTool('iris_inspect', { ref: q.ref })).text);
...
const summary = {
  dimension: 'Source localization — component render stack + source (Iris-only on the stack)',
  ...
  competitor_ceiling: {
    ...
    source:
      'file:line only IF the element carries a data-iris-source attr (DOM-readable); else none',
    locator: 'a CSS selector / role+name — not a source location',
  },
  note: 'The component stack (e.g. ["DeployTable","Deployments","App"]) is fiber-derived and Iris-only. Raw source file:line is parity where the babel plugin stamped data-iris-source. For an agent that must edit the code, Iris collapses find-the-component to one call.',
};
writeFileSync('bench/raw/source-localize.json', JSON.stringify(summary, null, 2));
for (const r of rows) console.log(JSON.stringify(r));
console.log(
  `\n=== source-localize: Iris component-stack ${localized.length}/${rows.length}, source ${withSource.length}/${rows.length}; competitor stack 0/${rows.length} (fiber unreachable) ===`,
);
process.exit(0);
