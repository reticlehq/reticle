...
// together, over and over. iris_flow_verify replays ALL saved flows deterministically and returns ONE
...
// Self-contained golden-path flows (each includes login), recorded once and saved to .iris/flows/.
const FLOWS = [
...
// Record flows POST-LOGIN (login is NOT part of the flow): iris_flow_verify replays the suite
...
async function verifySuite(names) {
  ...
  try {
    await a.login();
    await sleep(600);
    const res = await a.c.callTool('iris_flow_verify', { names });
    const text = res.text || '';
    let obj = {};
    try {
      obj = JSON.parse(text);
...
const summary = {
  layer: 'C-suite (suite-scale RRE — iris_flow_verify consolidated verdict)',
  metric: 'tokens an agent/CI READS to re-verify a K-flow suite, per run',
  points,
  note: 'iris_flow_verify returns ONE verdict for the whole suite (passing flows counted, only failures detailed) → read-cost ~constant in K. Competitors re-drive each flow with the LLM (~30,249 tok/flow) → cost is K*per-flow. The ratio therefore GROWS with suite size: the chased RRE metric compounds.',
};
writeFileSync('bench/raw/suite-rre.json', JSON.stringify(summary, null, 2));
const last = points.at(-1);
console.log(
  `\n=== suite-RRE: ${names.length} flows verified in ~${last?.iris_verify_tokens} tok (${last?.status}) vs ${last?.competitor_redrive_tokens} re-drive => ${last?.suite_rre_ratio}x (grows with K) ===`,
);
process.exit(0);
