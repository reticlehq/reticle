// Time-travel (Reticle-only capability): verify a TIME-GATED flow at clock speed, not wall-clock speed.
//
// The demo's createDeployment sets a new deployment to status 'building', then a 2600ms setTimeout
// flips it to 'live'. Reticle patches the app's setTimeout/Date, so it can FREEZE the clock, run the
...
// → slow. The timer duration is their structural floor; Reticle's is ~0. We show both paths with Reticle:
...
async function fill(a, testid, value) {
  ...
  if (q.ref) await a.c.callTool('reticle_act', { ref: q.ref, action: 'fill', args: { value } });
}
// `new-deploy` / `deploy-submit` trip Reticle's destructive-action guard (a deploy is consequential),
// so the click needs explicit confirmation — itself an Reticle-only safety the harness opts through.
...
async function statusOfTop(a) {
  const st = await a.c.callTool('reticle_state', { store: 'app', path: 'deployments.0.status' });
  return {
...
async function run() {
  ...
  const advanced = await (async () => {
    ...
    try {
      ...
      await a.c.callTool('reticle_clock', { freeze: true });
      await clickConfirm(a, 'deploy-submit');
      await sleep(150);
      const t0 = monotonic();
      await a.c.callTool('reticle_clock', { advanceMs: TIMER_MS });
...
console.log(
  `\n=== time-travel: Reticle verifies building→live in ~${r.advanced.wall_ms}ms via clock-advance (status=${r.advanced.status}) vs ~${r.realWait.wall_ms}ms real-wait floor => ${speedup}x faster, deterministic ===`,
);
process.exit(0);
