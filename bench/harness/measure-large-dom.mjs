...
// huge — because Iris reads the consequence, not the whole tree." That only shows where a full
...
//   - the full page snapshot (iris_snapshot scope:page)   — the baseline a screenshot/whole-tree tool pays
//   - the verify loop (query -> act_and_wait -> assert)   — what Iris actually pays per check
// Run apps/large-dom-bench first:  pnpm --filter @syrin/iris-large-dom-bench dev   (defaults to :4313)
...
const PORT = process.env.LARGE_DOM_IRIS_PORT ?? '4456';
const ROW = process.env.LARGE_DOM_ROW ?? '42';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

function refFromQuery(text, testid) {
  // iris_query returns JSON descriptors carrying ref + testid; pull the ref for our testid.
  try {
...
await c.callTool('iris_wait_ready', { timeout_ms: 15000 }).catch(() => undefined);
await sleep(300);
...
rec('iris_snapshot(page)', await c.callTool('iris_snapshot', { scope: 'page' }));
...
for (let i = 0; i < 20; i += 1) {
  q = await c.callTool('iris_query', { by: 'testid', value: approveId });
  ref = refFromQuery(q.text ?? '', approveId);
  if (ref) break;
  await sleep(200);
}
rec('iris_query(testid)', q);
if (ref) {
  rec(
    'iris_act_and_wait',
    await c.callTool('iris_act_and_wait', {
      ref,
      action: 'click',
  ...
  rec(
    'iris_assert(signal)',
    await c.callTool('iris_assert', { predicate: { kind: 'signal', name: 'row:approved' } }),
  );
}
await c.stop();

const snapshot = rows.find((r) => r.call.startsWith('iris_snapshot'))?.tokens_o200k ?? 0;
const loop = rows
  .filter((r) => r.call !== 'iris_snapshot(page)')
