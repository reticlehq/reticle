// Wasted-render storm benchmark (Iris-only): a page can be thrashing — committing many React renders
// a second — while the DOM stays visually identical. To a screenshot/DOM tool the page is idle; only
// Iris's commit meter (iris_state __iris_renders) sees it. We measure React commits over a 1s window
...
const url = (storm) =>
  storm ? `${BASE}${BASE.includes('?') ? '&' : '?'}iris-bug=render-storm` : BASE;

async function commitRate(storm) {
  const a = new IrisAdapter(url(storm));
  await a.start();
  try {
    await a.login();
    await sleep(400);
    const read = async () => {
      const r = await a.c.callTool('iris_state', { store: '__iris_renders', path: 'commits' });
...
const summary = {
  dimension: 'Wasted-render storm — React commit rate (Iris-only)',
  window_ms: WINDOW_MS,
  idle_commits: idle.commits_in_window,
  storm_commits: storm.commits_in_window,
  iris_read_tokens: storm.read_tokens,
  competitor:
  ...
  note: "Idle is not zero — the demo's count-up animations re-render ambiently (~30-40/s, and those DO mutate the DOM, so a DOM tool can see them). The storm adds ~60/s of PURE-WASTE commits (series redraws identically → no DOM mutation), nearly tripling the commit rate. Iris reads the count in one iris_state call; an outside tool sees no new DOM change from the storm.",
};
writeFileSync('bench/raw/render-storm-bench.json', JSON.stringify(summary, null, 2));
console.log(JSON.stringify(summary, null, 2));
console.log(
  `\n=== render-storm: idle ${idle.commits_in_window} commits/s vs storm ${storm.commits_in_window} commits/s in ${WINDOW_MS}ms — Iris ${summary.detected ? 'DETECTED' : 'did NOT detect'} @ ~${storm.read_tokens} tok; competitors see an idle DOM ===`,
);
process.exit(0);
