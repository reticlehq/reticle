...
**looks done but isn't**, so a screenshot/"does it look right" check sails right past it.
...
Every one of these is **proven against Iris's real verdict logic** in
`packages/server/src/runs/generated-app-bugs.test.ts` (runs in CI, no browser). This app is the live,
clickable counterpart.

## Verify it with Iris (live)

The app already serves a small `window.__app` store (`expenses`, `total`) — the seam an instrumented
build exposes for state-truth checks.
