...
import { start, TOOLS, BaselineStore, RecordingStore, LaunchedRealInputProvider } from '@iris/server';
...
console.log('\n=== iris drive (launched browser) vs smart-sentence, headless ===');
chk('iris drive launched a browser + the app SDK connected', server.bridge.sessions.count()>0, `sessions=${server.bridge.sessions.count()}`);
...
const hov=await T('iris_act',{ref,action:'hover'});
chk('iris_act{hover} reports inputMode:"real" via the launched browser', hov.inputMode==='real', `inputMode=${hov.inputMode}`);
await sleep(750);
...
const words=(await T('iris_query',{by:'testid',value:'word:0'})).elements?.length??0;
chk('hover-gated words mount headless (no foreground tab needed)', words>0, `word:0=${words}`);
console.log(`\n${fail===0?'✅ P2 IRIS DRIVE VERIFIED':'❌ FAILED'} (${pass} passed, ${fail} failed)`);
