...
const { TOOL_SURFACE } = await import(pathToFileURL(path.join(ROOT, 'server/dist/tools/tool-surface.js')).href);
const { advertisedTools } = await import(pathToFileURL(path.join(ROOT, 'server/dist/mcp/mcp.js')).href);
...
spawnSync('node', ['server/dist/cli.js', 'stop', '--port', PORT, '--quiet'], { cwd: ROOT });
