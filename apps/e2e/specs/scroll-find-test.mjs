...
// the visible window of 500 rows, so a plain iris_query for an off-screen row finds nothing;
// iris_scroll_to must scroll the container until that row mounts, then return it.
...
console.log('\n=== N5 SCROLLFIND: iris_scroll_to reveals a virtualized off-screen row ===');
chk('app SDK connected', server.bridge.sessions.count() > 0);

const TARGET = 'row-40';
const before = await T('iris_query', { by: 'testid', value: TARGET });
chk('the off-screen row is NOT rendered initially (virtualized)', (before.elements?.length ?? 0) === 0);

const container = (await T('iris_query', { by: 'testid', value: 'virtual-list' })).elements?.[0]?.ref;
chk('found the list container ref', typeof container === 'string', container);

const found = await T('iris_scroll_to', { by: 'testid', value: TARGET, container, maxScrolls: 40 });
chk('iris_scroll_to revealed the row by scrolling', found.found === true, `scrolls=${found.scrolls}`);
...
const missing = await T('iris_scroll_to', { by: 'testid', value: 'row-99999', container, maxScrolls: 120 });
