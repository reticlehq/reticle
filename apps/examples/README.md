...
**Both are currently ungated** — tracked in `server/src/tools/integration-coverage.test.ts`, which pins the gap so it stays visible. An example that nothing runs is a promise nobody checks.
