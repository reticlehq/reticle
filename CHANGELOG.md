...
- **`@reticlehq/vite-plugin` — host-routed Vite apps can opt in to non-localhost connects.** The browser SDK already accepts `allowNonLocalhost`, but the documented one-line Vite integration could not pass it through, so apps that must run at dev hostnames such as `app.localtest` were unreachable even though the SDK supported the mode. The plugin now forwards `allowNonLocalhost: true` into the injected `connect()` call and also accepts `VITE_RETICLE_ALLOW_NON_LOCALHOST=1` for one-session debugging; the pairing token requirement remains in place.
