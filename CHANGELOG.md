...
- **`@reticlehq/server` — no-session diagnosis survives daemon restarts.** The daemon now records when a project has previously served a browser session, so a restart reports lost session state instead of claiming it has never seen one.
