...
import { SESSION_AUTO } from '@reticle/protocol';
...
describe('reticleParamsFromSearch', () => {
  it('extracts session and projectId from namespaced params', () => {
    const search = `?${RETICLE_URL_PARAM.SESSION}=lease-7&${RETICLE_URL_PARAM.PROJECT}=acme-9f3c`;
    expect(reticleParamsFromSearch(search)).toEqual({ session: 'lease-7', projectId: 'acme-9f3c' });
  });

  it('returns an empty object when the params are absent', () => {
    expect(reticleParamsFromSearch('?foo=bar&page=2')).toEqual({});
    expect(reticleParamsFromSearch('')).toEqual({});
  });

  it('ignores empty values', () => {
    expect(reticleParamsFromSearch(`?${RETICLE_URL_PARAM.SESSION}=`)).toEqual({});
  });

  it('does not clash with the app’s own session param', () => {
    // A plain ?session= belongs to the app, not Reticle — only the namespaced one is read.
    expect(reticleParamsFromSearch('?session=app-thing')).toEqual({});
  });

  it('reads one without the other', () => {
    expect(reticleParamsFromSearch(`?${RETICLE_URL_PARAM.PROJECT}=just-project`)).toEqual({
...
describe('resolveConnectIdentity', () => {
  const search = `?${RETICLE_URL_PARAM.SESSION}=lease-7&${RETICLE_URL_PARAM.PROJECT}=acme`;
