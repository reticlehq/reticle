'use strict';
// Webpack pre-loader for `@syrin/iris/next` — ships INTO the umbrella's dist/ alongside next.js and
// babel.js. Applies the Iris source-mapping babel transform (stamps data-iris-source) to the project's
// JSX/TSX, then hands the result to next-swc-loader. SWC still does the real compile; we only stamp
// source locations. Dev-only (wired by withIris).
//
// Authored as plain CJS (module.exports = function) so webpack requires it directly. The babel plugin
// is NOT imported from the private @syrin/iris-babel-plugin — it's pulled from the umbrella's own
