...
- **Source mapping** — stamps `data-reticle-source="file:line:col"` on JSX host elements (via [`@reticle/babel-plugin`](https://www.npmjs.com/package/@reticle/babel-plugin)) so `reticle_inspect` can report the component's source file — needed on React 19.
...
That is the entire integration — no entry-file edit, no Babel-plugin wiring, no env gating. `npx @reticle/core init` adds this line for you automatically in a Vite project.
