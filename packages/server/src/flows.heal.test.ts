...
describe('FlowStore.heal + iris_flow_heal (M8 Stage B SELFHEAL)', () => {
  let root: string;
  let store: FlowStore;
  ...
  it('S2.1: heal apply:true rewrites a renamed testid and a subsequent replay is green', async () => {
  ...
  it('S2.2: heal apply:false returns the proposal but does NOT modify the file', async () => {
    await store.saveFlow(flowFile('chat', [clickStep('old-id')]));
  ...
  it('S2.3: drift with no confident nearest leaves file untouched with a clear status', async () => {
    await store.saveFlow(flowFile('chat', [clickStep('old-id')]));
    const before = await readFile(flowPath(root, 'chat'), 'utf8');
  ...
  it('S2.4: heal on a green flow is a no-op "nothing to heal"', async () => {
  ...
  it('S2.5: heal on a missing flow returns a structured error', async () => {
  ...
  it('S2.6: a path-traversal name is rejected before any disk op', async () => {
  ...
  it('S2.7: apply rewrites only confident steps, leaves low-confidence steps', async () => {
  ...
  it('S2.8: heal on a malformed flow file returns PARSE_FAILED, no write', async () => {
    const fsPort = createNodeFileSystem();
  ...
  it('S2.9: heal apply:true then heal again is nothing_to_heal (idempotent)', async () => {
    await store.saveFlow(flowFile('chat', [clickStep('old-id')]));
  ...
  it('S2.10: healed file is re-readable, byte-stable, preserves createdAt + trailing newline', async () => {
    await store.saveFlow(flowFile('chat', [clickStep('old-id')]));
...
describe('FlowStore.heal — writer (SELFHEAL)', () => {
  let root: string;
  let store: FlowStore;
