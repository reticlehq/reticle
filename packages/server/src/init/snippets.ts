 ...
 * where a bare `@reticle/core` import resolves. A bare import in a plain index.html does NOT resolve in
...
export function htmlManual(port: number | undefined, projectId?: string): string {
  ...
  • Bundled app (Create React App, webpack, Parcel, Vue/Svelte CLI, etc.) — add to your ENTRY module
    (e.g. src/index.js or src/main.js), where '@reticle/core' resolves through your bundler:

      if (location.hostname === 'localhost') {
        const { reticle, install } = await import('@reticle/core');
        install();
        reticle.connect(${arg});
      }

  • Plain static HTML with no build step — the browser can't resolve the bare '@reticle/core' import, so
