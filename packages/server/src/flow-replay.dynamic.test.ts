...
/**
 * M8 Stage B ANNOTATE (G) — replay must SKIP asserting a `dynamic`-marked region (the LLM-output
...
describe('replayFlow dynamic-region skip (M8 Stage B ANNOTATE)', () => {
  it('G1: a dynamic-marked expect testid is NOT asserted (absent region does not fail replay)', async () => {
  ...
  it('G2: a NON-dynamic missing expect testid still drifts (skip is scoped to dynamic only)', async () => {
  ...
  it('G3: the dynamic skip does NOT stop the step action from running', async () => {
    const session = new FakeSession(new Set(['send']));
    await replayFlow(
      session,
      flow([step('send', 'caption-text')], ['caption-text']),
