/**
 * IrisRunner — the programmatic Replay/Verify API a host platform (OEM/design partner) or CI drives
 * from its own pipeline, with no MCP stdio and no human. It reuses the existing flow-replay machinery
 * and the verification-run assembler, returning a stable IrisVerificationRun.
 *
...
/** The live capabilities IrisRunner needs. Injected so tests pass fakes (no CDP, no session). */
...
export class IrisRunner {
   ...
   * DOM (the same reason iris_flow_verify is sequential).
   */
  async verify(opts: VerifyOptions): Promise<IrisVerificationRun> {
