...
/** Minimal ToolDeps stub: only `sessions.list()` is exercised by iris_sessions. */
...
describe('createToolInvoker', () => {
  ...
  it('an unknown tool name rejects with a named error', async () => {
    const invoke = createToolInvoker(fakeDeps());
    await expect(invoke('iris_nope', {})).rejects.toThrow(UNKNOWN_TOOL_ERROR);
  });

  it('a tool handler error propagates (not swallowed)', async () => {
    // iris_diff throws when the baseline does not exist — a handler error path.
    ...
    await expect(invoke(IrisTool.DIFF, { baseline: 'missing' })).rejects.toThrow();
  });
});
