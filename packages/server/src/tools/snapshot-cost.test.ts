 ...
 * asserted as regression guards AND printed (see the test output) so the docs can quote them.
 *
...
describe('snapshot token cost (efficiency wins, measured on real functions)', () => {
  it('a diff after a one-row change costs a tiny fraction of a full re-snapshot', () => {
    ...
    process.stdout.write(
      `[snapshot-cost] full re-snapshot=${String(fullTokens)} tok  diff=${String(deltaTokens)} tok  saved=${String(savedPct)}%\n`,
    );
  ...
  it('an unchanged re-snapshot collapses to near-zero tokens', () => {
    ...
    process.stdout.write(`[snapshot-cost] unchanged re-snapshot=${String(unchangedTokens)} tok\n`);
  ...
  it('cost preview reports the full size up front so the agent can bail before reading', () => {
    ...
    process.stdout.write(
      `[snapshot-cost] cost preview: ${String(preview.tokens)} tok / ${String(preview.bytes)} bytes\n`,
    );
