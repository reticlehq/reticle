...
The [Reticle](https://github.com/reticlehq/reticle) bridge + MCP server. It hosts a localhost WebSocket endpoint your app's `@reticle/browser` SDK connects to, and exposes MCP tools your coding agent uses to look at, act on, observe, and assert against the live app.

```bash
npx @reticle/server        # bridge on ws://localhost:4400, MCP over stdio
```

Point your agent at it (e.g. Claude Code `.mcp.json`):

```jsonc
{ "mcpServers": { "reticle": { "command": "npx", "args": ["@reticle/server", "mcp"] } } }
```
