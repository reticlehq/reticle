...
A predicate declares what should be true, as `{ kind, ...fields }`. Reticle has thirteen kinds: `signal`, `state`, `net`, `route`, `element`, `text`, `console`, `animation`, `style`, `settled`, and the combinators `allOf`, `anyOf` and `not`. Which one you pick decides whether a green means anything.
...
| `style`            | medium    | The browser resolved these CSS properties              |
...
Computed style via `getComputedStyle` — what the browser resolved, not what the stylesheet said. A design-system token change that leaves the same class on the card cannot produce a verdict from `element`/`text`; this can.

Values are compared in the browser's normalised form (`rgb(...)`, `px`). Exact match for colors. `{ "contains": "…" }` or `{ "matches": "…" }` (a regex) for compound values like `box-shadow` and `font-family`. CamelCase (`backgroundColor`) and kebab-case (`background-color`) both work. A miss names the property and both sides.

This does not persist into saved flows — same as `route` and `animation`.
