...
`satisfies` asserts a PROPERTY of the rendered text instead of its exact bytes: the same shape and the same properties [`state` accepts](#satisfies-asserting-a-property-instead-of-exact-bytes), applied where the user actually reads the value:

```json
{
  "kind": "text",
  "scope": "[data-testid=\"release-note\"]",
  "self": true,
  "satisfies": { "property": "nonEmpty" }
}
```

That is the assertion a generated feature can pass twice. `contains` demands bytes a model's output does not repeat, so it either fails on correct output or gets weakened to a word so common it proves nothing; "the box rendered and there is something in it" is the failure the feature actually has.
...
```json
{ "kind": "state", "store": "app", "path": "summary.text", "satisfies": { "property": "nonEmpty" } }
```

| `property` | Extra fields | Holds when |
| --- | --- | --- |
| `nonEmpty` | none | it produced something at all, the honest floor for any output |
| `oneOf` | `values` | it landed inside the allowed set, exact membership, no coercion |
| `withinTolerance` | `of`, `tolerance` | a number near enough to the expected one, inclusive on the bound |
| `matchesPattern` | `pattern` | its string form matches that regular expression |
| `type` | `is` | it is that kind of thing; `array` and `object` are distinguished, which `typeof` cannot do |
  ...
  "predicates": [
    {
      "kind": "state",
      "path": "invoice.total",
      "satisfies": { "property": "withinTolerance", "of": 274.58, "tolerance": 0.01 }
    },
    {
      "kind": "state",
      "path": "invoice.status",
      "satisfies": { "property": "oneOf", "values": ["draft", "sent"] }
    },
    { "kind": "state", "path": "invoice.lines", "satisfies": { "property": "type", "is": "array" } }
  ]
}
```
...
| property    | holds when                  |
| ----------- | --------------------------- |
| `unchanged` | the value is the one it was |
| `changed`   | it is not                   |
| `increased` | the number went up          |
| `decreased` | the number went down        |

```json
{
  "kind": "state",
  "path": "cart.total",
  "satisfies": { "property": "decreased", "by": { "op": "equals", "value": 12 } }
}
```

`increased` and `decreased` take an optional `by`, in the protocol's own comparison vocabulary: `op` is `equals`, `at-least` or `at-most`, with an optional `tolerance` on all three. Without `by`, any movement in the right direction holds.
...
<Warning>
A combinator needs its own `kind`, and the children go in `predicates`, or in `of`, which is
accepted as an alias for it. `not` takes a single child in `predicate`, or in `of`. A bare
...
| `since` | `net`, `route`, `console`, `animation` and `signal` leaves, and the call itself | Only consider events after this cursor |
