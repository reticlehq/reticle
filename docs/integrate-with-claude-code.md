...
1. MCP: register the "reticle" server ONCE, globally (user scope) — not per project:
   ...
   (`npx @reticle/core init` runs this for you. Use a project-scoped `.mcp.json` only if a repo
...
2. Install the dev deps from the local registry (skip the registry lines if using public npm):
   ...
   npm i -D @reticle/core        # one package: SDK + React adapter + plugins + test runner + server
...
4. React source mapping (so failures point at file:line) — getting-started Step 3:
   - install the React adapter: import { install } from '@reticle/core'; install()
   - Vite: add @reticle/babel-plugin to vite.config; Next: wrap next.config with withReticle from @reticle/next
