...
| **Preact** | as Vite | no | no | yes | Sensor, React adapter via `preact/compat` |
...
**Source pointers work, and that is measured rather than assumed.** The stamper is JSX-syntactic: it tags a host element by its tag name and reads nothing about React, so a Preact `.jsx` or `.tsx` file is stamped exactly as a React one is. `class` instead of `className`, a `@jsxImportSource preact` pragma and a `@preact/signals` value in the tree all leave it unmoved. There are tests for each of those in `packages/babel-plugin`, so a verdict on a Preact app names a `file:line` like any other.
