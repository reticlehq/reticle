...
| **A verdict-producing tool** | Add it to `VERDICT_TOOLS` (`packages/server/src/agent/tools/feedback-tools.ts`). Otherwise it emits no `verification_completed` and stops counting toward the product's headline metric | ✓ |
