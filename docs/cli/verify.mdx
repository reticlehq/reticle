...
```bash
npx @reticlehq/server verify <url> [--headed] [--timeout N] [--storage-state <file>] [--expect <json> | --expect-file <path>]
```
...
| `--expect <json>` | string | unset | A JSON predicate graded against the tab a running daemon already owns. Exit 0 only on a verified pass. Cannot combine with `--storage-state` |
| `--expect-file <path>` | string | unset | Same as `--expect`, but read the JSON from a file. Prefer this on Windows PowerShell: `npx.cmd` re-parses `--expect '{...}'` and strips the inner quotes |

On Windows PowerShell, write the predicate to a file and pass `--expect-file` so the JSON never goes through shell quoting:
