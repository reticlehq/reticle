...
Sends all existing saved flows and run artifacts from `.reticle` to the linked project, honoring the `sync.flows` and `sync.runs` policy. Prints totals plus separate `flows` and `runs` counts, so bootstrap and CI can prove the whole local suite reached the cloud.
