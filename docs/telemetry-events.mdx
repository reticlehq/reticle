...
Every event Reticle emits is listed below with the same three things: **when it fires**, **what it carries**, and **why it exists**. The authoritative list is `TelemetryEventKind` in `core/src/telemetry.ts`; if this page and that file disagree, the file wins and this page is a bug.

Source of truth for everything below:

| File | Defines |
| --- | --- |
| `core/src/telemetry.ts` | `TelemetryEventKind`, the envelope, and most payload blocks |
| `core/src/telemetry-session.ts` | `SessionSummarySchema`, `ProjectProfileSchema`, `MachineSnapshotSchema` |
| `core/src/telemetry-feedback.ts` | `FeedbackSchema`, `IdentitySchema`, and the enums they use |
...
| `event` | `TelemetryEventKind` | yes | Which kind this is. The full list is `TelemetryEventKind` in `core/src/telemetry.ts` |
...
**Wire name:** `reticle_installed`. **Emitted from:** `server/src/telemetry/cli-telemetry.ts:51`. **Session-scoped:** no. **Payload block:** none.
...
**Wire name:** `cli_command_run`. **Emitted from:** `server/src/telemetry/cli-telemetry.ts:58`. **Session-scoped:** no. **Payload:** two envelope fields.
...
**Wire name:** `daemon_started`. **Emitted from:** `server/src/telemetry/daemon-telemetry.ts:75`. **Session-scoped:** yes. **Payload block:** none.
...
**Wire name:** `daemon_stopped`. **Emitted from:** `server/src/telemetry/daemon-telemetry.ts:138`. **Session-scoped:** yes. **Payload block:** `session`, a `SessionSummary` with `final: true`.
...
**Wire name:** `session_progress`. **Emitted from:** `server/src/telemetry/daemon-telemetry.ts:116`. **Session-scoped:** yes. **Payload block:** `session`, the same `SessionSummary` shape with `final: false` and no `exit`.
...
**Wire name:** `verification_completed`. **Emitted from:** `server/src/tools/invoke-tool.ts:144` and `server/src/telemetry/run-telemetry.ts:39`. **Session-scoped:** yes. **Payload block:** `verification`.
...
**Wire name:** `project_profiled`. **Emitted from:** `server/src/telemetry/daemon-telemetry.ts:91`. **Session-scoped:** yes. **Payload block:** `project`. Once per daemon start, so it is cheap.
...
**Wire name:** `version_changed`. **Emitted from:** `server/src/update/updater.ts:35`. **Session-scoped:** no. **Payload block:** `versionChange`.
...
**Wire name:** `runtime_crashed`. **Emitted from:** `server/src/daemon/daemon-resilience.ts:155`. **Session-scoped:** yes. **Payload block:** `crash`.
...
**Wire name:** `feedback_submitted`. **Emitted from:** `server/src/telemetry/feedback.ts:256` and `:270`. **Session-scoped:** yes. **Payload block:** `feedback`.
...
**Wire name:** `identified`. **Emitted from:** `server/src/telemetry/identify.ts:77`. **Session-scoped:** no. **Payload block:** `identity`.
...
**Wire name:** `mcp_client_connected`. **Emitted from:** `server/src/telemetry/mcp-connection.ts:29`. **Session-scoped:** yes. **Payload block:** `connection`.
...
**Wire name:** `app_instrumented`. **Emitted from:** `server/src/telemetry/app-instrumented.ts:55`. **Session-scoped:** yes. **Payload block:** `instrumentation`.
...
**Wire name:** `mcp_connection_lost`. **Emitted from:** `server/src/mcp/mcp-outage.ts:53`. **Session-scoped:** no. **Payload block:** `outage`.
...
**Wire name:** `init_completed`. **Emitted from:** `server/src/telemetry/init-telemetry.ts:32`. **Session-scoped:** no. **Payload block:** `init`.
...
**Wire name:** `bug_found`. **Emitted from:** `server/src/tools/invoke-tool.ts:166` and `server/src/telemetry/run-telemetry.ts:59`. **Session-scoped:** yes. **Payload block:** `bug`.
...
**Wire name:** `tool_refused`. **Emitted from:** `server/src/telemetry/tool-refused.ts:49`, reached from four call sites in `server/src/tools/invoke-tool.ts` (`:285` a handler threw, `:325` the wrong-tab guard, `:346` a message-shaped refusal, `:376` the `{error}` return convention). **Session-scoped:** yes. **Payload block:** `refusal`.
...
**Wire name:** `onboarding_step`. **Emitted from:** `server/src/telemetry/onboarding-funnel.ts:22`, reached from the CLI (`command/cli-onboarding.ts`), the installer (`command/setup/setup-install.ts`, `command/setup/setup-mcp.ts`), `init` (`init/src/host.ts` through the injected `InitHost`) and the tool dispatcher (`surface/tools/invoke-tool.ts`). **Session-scoped:** no. **Payload block:** `onboarding`.

| Field | Type | Meaning |
| --- | --- | --- |
| `phase` | `OnboardingPhase` | `install`, `onboard` or `first_run` |
| `step` | `z.enum` over every name in `OnboardingSteps` | A closed set, derived from the table rather than retyped |
| `status` | `OnboardingStepStatus` | `started`, `completed`, `failed`, `abandoned` or `skipped` |
| `elapsedMs` | int, optional | How long it took. Absent means NOT MEASURED, never zero |
| `reason` | string, up to 64, optional | Classified cause on failure. Our vocabulary, never a raw error or a path |
| `stack` | string, up to 64, optional | The framework, so a phase failing on only one is visible |
| `unattended` | boolean, optional | Nobody was asked anything: the zero-human-input path an agent takes from a link |

**The question it answers:** where do people stop, between never having heard of Reticle and having a verdict on their own app. No other event can answer it. `init_completed` fires when the files are written, and the story continues for minutes after that; `app_instrumented` sees one moment of it. This is the only event that spans the whole route.

The steps are the funnel, in order, and they live in one table in `core/src/onboarding.ts`:

| Phase | Steps, in the sequence they occur |
| --- | --- |
...
`first_run.verdict_produced` is the conversion event. Everything before it is setup that proved nothing.

Four decisions in this shape are worth knowing before you add to it:

- **One kind for every step, not a kind per step.** A phase plus a step keeps a funnel a `GROUP BY` instead of a union, so inserting or renaming a step does not silently break the queries watching it, and a new step appears in the existing chart rather than beside it.
- **`step` is a closed `z.enum`, not a capped string.** It was `z.string().max(48)`, with a comment promising it was never user text, and a cap is not a promise. `/Users/someone/secret/project` is 28 characters and would have validated cleanly, out of the one payload a person can edit: the installer's breadcrumb file, in their own home directory. The enum is derived from `OnboardingSteps`, so a step added there is accepted on the same edit and a renamed step cannot arrive under both spellings.
- **`abandoned` is a status of its own.** A step that is `started` and never resolved is indistinguishable from one that was never reached, and both read as "we never got there". Somebody walking away from a step we know they began is a different fact from the step failing, and only one of those is our bug. `reportStepTimed`'s `finally` is what makes it reliable: an exception escaping without a report turns a failure into silence.
- **`skipped` is neither a loss nor a win.** Already installed, already instrumented. Counting it as either makes a re-run look like a conversion or like a drop.

A malformed step is **dropped**, not sent and not thrown. Sending it would put unvalidated text on the wire; throwing would let a metric change what a user's install does.
...
1. Add the kind to `TelemetryEventKind` in `core/src/telemetry.ts`, with a doc comment saying what question it answers.
...
6. If it produces a verdict, add the tool to `VERDICT_TOOLS` (`server/src/tools/feedback-tools.ts`). If it introduces a finding kind, add that to core's enum and never re-list it locally.
