...
### The dev server runs in a container

Everything below the CLI assumes the daemon and the dev server share a filesystem. Three things break when they do not (Docker, a devcontainer, WSL, or a dev server on another machine), and all three look identical from the page: the SDK loads, the socket opens, and no session appears.

**1. The pairing token.** The daemon writes `~/.reticle/pairing-token` on the host. The build plugin reads it from `$HOME` _inside_ the container, finds nothing, mints its own, and inlines that into every page, which the bridge then refuses. The console says `bridge refused the connection: authentication failed`, and `reticle status` names this case in full.

Mount the host's token read-only and point the plugin at it:

```yaml
frontend:
  environment:
    RETICLE_PAIRING_TOKEN_DIR: /reticle-state
  volumes:
    - ~/.reticle/pairing-token:/reticle-state/pairing-token:ro
```

Run the daemon at least once first, or Docker creates a _directory_ at that host path. The token is read when the dev server resolves its config and inlined then, so restart the dev server afterwards.

**2. The projectId.** The plugin reads it from `.reticle.json`, the file `reticle init` writes. If that file is not visible inside the container (it is not in the build context, or the source is not mounted), the plugin falls back to deriving one from the absolute project root. That is `/app` inside and your host path outside, so the two disagree. Make sure `.reticle.json` is mounted with the source, or pass the id explicitly: `reticle({ projectId: 'your-id-from-.reticle.json' })`.

**3. The dependencies.** A container that installs its own `node_modules` into an anonymous volume will not see the two packages `init` added by restarting. Rebuild the image and renew the volume:

```bash
docker compose build frontend && docker compose up -d --renew-anon-volumes frontend
```

`reticle init` prints all three as a notice when it finds a `Dockerfile` or a compose file near the app, so on a fresh install you should meet this page having already been told.
...
```json
"feedback_invite": "Reticle could not tell what happened here. If you expected otherwise, reticle_session { action: "feedback" }. An unknown verdict is our defect, not yours"
```
...
- `reticle_look { action: "page" } { mode: "interactive" }` shows everything actionable.
...
## `reticle_look { action: "state" }` returns nothing

`storeNames` empty means no store was registered. `reticle_look { action: "state" }` reads what your app registered in `src/reticle-dev.ts`.
...
Some clients read their MCP tool list once, at startup, and never again. Cursor Cloud Agent is the usual report: the daemon is listening on port 4400, the app is instrumented, and `reticle_*` is absent for the whole session. Restarting the client picks the tools up when a restart is possible. When it is not, the same tools are on the HTTP transport.

`GET /mcp/sse` opens the session. The first SSE frame is an `endpoint` event whose data is the path to POST to, including the session id. POST JSON-RPC there. The POST answers `202` with an empty body, and the result comes back on the SSE stream. The handshake and a working client are in [HTTP transport](/http-transport).
...
<AccordionGroup>
  <Accordion title="Why does reticle_session { action: "list" } return nothing when my app is clearly running?">
...
<Accordion title="Does a verdict of unknown mean the test failed?">
  ...
  that is a Reticle defect worth sending with `reticle_session { action: "feedback" }`.
</Accordion>
