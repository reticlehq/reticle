...
The nine tools you are shown. There is no `reticle_run` on this surface: these nine plus their actions are the whole of what you can call.
...
- **One tool surface, and it is closed.** Reticle advertises nine tools: navigate/act/act_and_wait/assert/look/observe/session/verify, plus `reticle_tools`. That is the whole detect loop, the file-pointer, the feedback channel, the handback, and the one tool that CONCLUDES, reached by an `action` argument rather than by a name each. `reticle_tools` discovers: no args lists every tool name and summary, `names:[…]` loads full params on demand. There is **no `reticle_run`** here, so a tool that is not in the nine is not reachable by any call: recording a flow, screenshots, visual diff, storage, network mocking and the clock all need a daemon started with `RETICLE_ADVERTISE_ALL_TOOLS=1` (read at startup, so restart it), which advertises the full table with output schemas. `reticle_verify` is advertised: `{ action:"flows" }` replays every saved flow with no model in the loop, and `{ action:"crawl" }` drives every reachable control itself and reports the whole fault set in one call. **Sizes are deliberately not quoted here.** A count in prose goes stale, and has three times already. `reticle_tools` reports the live surface, and SKILL.md carries the one gated table.
