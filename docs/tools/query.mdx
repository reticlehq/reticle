...
<Warning>
  **reticle_query is not advertised.** The default surface is the merged nine, so an agent does not see
...
| `attrs` | Attribute NAMES to project per match, e.g. `['href']` to inventory links. The values come back in each match's `attrs` map. It does not filter, so `name=value` is refused |
