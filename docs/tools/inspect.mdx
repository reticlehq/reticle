...
<Warning>
  **reticle_inspect is not advertised.** The default surface is the merged ten, so an agent does not see
  this name. Call **`reticle_look { action: "element" }`** instead. Everything below describes what that call does;
