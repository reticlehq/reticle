 ...
 * `TypeError` wrote a session's `events.jsonl` until the disk filled, the tail of it one identical
 * stack over and over (#986).
 *
 * Nothing about that stream was informative after the first few occurrences. What a reader needs is
 * that the error happened, what it was, and that it is *still* happening — three facts, not hours
 * of duplicates.
 *
 ...
 * number of events over an unbounded session, which is exactly the case that filled a disk; and it makes
