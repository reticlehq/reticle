 ...
 *
 * THE RULE: one observer failing must never stop the others. Each install is wrapped, so the healthy
 * observers still run, the failure is reported over the bridge, and the caller gets a teardown list
 * of the same length. Built bare, a single `install*` that threw would take the whole `connect()`
 * down with it — and silently, because the SDK's own catch blocks swallow it.
 */
