...
describe('CSP violations reach the console channel', () => {
  ...
  const dispatchViolation = (
    ...
    Object.assign(event, { violatedDirective, blockedURI, disposition });
    document.dispatchEvent(event);
  };

  it('reports a report-only violation as a warning, because nothing was blocked', () => {
    const { emit, events } = collect();
    teardown = installConsole(emit);

    dispatchViolation('script-src', 'https://cdn.example.test/analytics.js', 'report');

    expect(events.filter((e) => e.type === EventType.CONSOLE_ERROR)).toHaveLength(0);
    const warn = events.find((e) => e.type === EventType.CONSOLE_WARN);
    expect(String(warn?.data['message'])).toContain('Report Only');
  });
