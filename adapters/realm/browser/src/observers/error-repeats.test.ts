...
describe('a repeating uncaught error is one fact', () => {
  ...
  it('reports order-of-magnitude checkpoints, carrying the count they stand for', () => {
    const limiter = createRepeatLimiter();
  ...
  it('costs O(log n) events for an error that never stops', () => {
    // The shape that matters: fourteen hours of one error is a handful of events, and the count
  ...
  it('does not suppress a first occurrence, however many others came before', () => {
    const limiter = createRepeatLimiter();
