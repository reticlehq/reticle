 ...
 *
 * One slide of the chat panel's carousel, which owns where it sits and how it is closed. A headline,
 * one short line and a button: no command to copy, which belongs in the terminal and the agent's
 * instructions, not on a card. The link comes from core's one discovery module, so this card, the CLI
 * and the agent can never point at different places.
 */
...
export const TALK_TEXT = {
  ...
  BODY: 'Tell us where Reticle got in your way.',
  BOOK: 'Book a call',
} as const;
