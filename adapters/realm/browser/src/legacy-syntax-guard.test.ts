...
const CORE_DIST = join(REPO_ROOT, 'core', 'dist');
const CORE_PACKAGE_JSON = join(REPO_ROOT, 'core', 'package.json');
const OPEN_VERIFICATION_DIST = join(REPO_ROOT, 'open-verification', 'dist');

/**
 * A tsconfig is JSONC, not JSON — this repo's own root config carries `/* … *\/` comments — and
 * `JSON.parse` throws on those verbatim. Strips `//` and block comments outside string literals
 * (tracking `"…"` so a comment marker inside a quoted value is never touched), which is all a
 * tsconfig needs: it has no other place `/` legitimately appears outside a string.
 */
...
/**
 * Webpack 4's bundled acorn parses up through ES2020's dynamic `import()` (its own code-splitting
 * mechanism, supported natively since webpack 2.1 — the browser SDK lazy-loads the presenter panel
 * through it) plus everything ES2019 already covers (object/array spread and rest, async iteration,
 * optional catch binding) without a bundler-config edit — issue #680 was specifically nullish
 * coalescing, optional chaining, and logical assignment, which ecmaVersion 2020 would ALSO accept if
 * this were the only guard against them. It is not: `MODERN_SYNTAX_TOKENS` above greps for exactly
 * those five tokens, in a separate test, so raising the parse ceiling to include dynamic `import()`
 * does not widen what this file accepts overall. `target: ES2017` in this repo's own tsconfigs is a
 * deliberately tighter, separately-enforced choice (the `resolves a target at or below ES2017` case
 * above) for syntax THIS repo controls; ES2020 here is the actual webpack-4 ceiling, which is what a
 * third-party dependency's own build (zod's) needs to clear. `.cjs` parses as a script
 * (`require`/`module.exports`, no top-level `import`/`export`); everything else parses as a module,
 * matching how these packages (and zod) ship ESM `.js` under `"type": "module"`.
 */
function parseEachUnderWebpack4Ceiling(dir: string): string[] {
  ...
  for (const file of jsFiles(dir)) {
    ...
    try {
      acorn.parse(readFileSync(file, 'utf8'), { ecmaVersion: 2020, sourceType });
    } catch (err) {
...
describe('published SDK dist stays parseable by webpack 4', () => {
  it.each([
    ...
    ['core', CORE_DIST],
    ['open-verification', OPEN_VERIFICATION_DIST],
  ] as const)(
  ...
  /** Each package's directory, repo-root-relative — the three links in core's transitive chain. */
  const PACKAGE_DIRS = {
    browser: join('adapters', 'realm', 'browser'),
    core: 'core',
    'open-verification': 'open-verification',
  } as const;

  it.each(['browser', 'core', 'open-verification'] as const)(
    '%s resolves a target at or below ES2017',
    (pkg) => {
      const base = parseJsonc(readFileSync(join(REPO_ROOT, 'tsconfig.base.json'), 'utf8')) as {
        compilerOptions?: { target?: string };
      };
      const config = parseJsonc(
        readFileSync(join(REPO_ROOT, PACKAGE_DIRS[pkg], 'tsconfig.json'), 'utf8'),
